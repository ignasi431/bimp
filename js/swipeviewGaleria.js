...
var SwipeView = (function(){
	var hasTouch = 'ontouchstart' in window,
		...
		SwipeView = function (el, options) {
			...
			this.contenedorImagenGaleria = typeof el == 'string' ? document.querySelector(el) : el;
			this.options = {
				text: null,
				numberOfPages: 3,
				snapThreshold: null,
			...
			this.contenedorImagenGaleria.style.overflow = 'hidden';
			this.contenedorImagenGaleria.style.position = 'relative';
			...
			this.contenedorImagenGaleria.appendChild(div);
			this.slider = div;
			...
			this.contenedorImagenGaleria.addEventListener(startEvent, this, false);
			this.contenedorImagenGaleria.addEventListener(moveEvent, this, false);
			this.contenedorImagenGaleria.addEventListener(endEvent, this, false);
			this.slider.addEventListener('webkitTransitionEnd', this, false);

/*			if (!hasTouch) {
				this.contenedorImagenGaleria.addEventListener('mouseout', this, false);
			}*/
		};
	...
	SwipeView.prototype = {
		...
		onFlip: function (fn) {
			this.contenedorImagenGaleria.addEventListener('swipeview-flip', fn, false);
			this.customEvents.push(['flip', fn]);
		},
		
		onMoveOut: function (fn) {
			this.contenedorImagenGaleria.addEventListener('swipeview-moveout', fn, false);
			this.customEvents.push(['moveout', fn]);
		},

		onMoveIn: function (fn) {
			this.contenedorImagenGaleria.addEventListener('swipeview-movein', fn, false);
			this.customEvents.push(['movein', fn]);
		},
		
		onTouchStart: function (fn) {
			this.contenedorImagenGaleria.addEventListener('swipeview-touchstart', fn, false);
			this.customEvents.push(['touchstart', fn]);
		},

		destroy: function () {
			while ( this.customEvents.length ) {
				this.contenedorImagenGaleria.removeEventListener('swipeview-' + this.customEvents[0][0], this.customEvents[0][1], false);
				this.customEvents.shift();
			}
			
			// Remove the event listeners
			window.removeEventListener(resizeEvent, this, false);
			this.contenedorImagenGaleria.removeEventListener(startEvent, this, false);
			this.contenedorImagenGaleria.removeEventListener(moveEvent, this, false);
			this.contenedorImagenGaleria.removeEventListener(endEvent, this, false);
			this.slider.removeEventListener('webkitTransitionEnd', this, false);

/*			if (!hasTouch) {
				this.contenedorImagenGaleria.removeEventListener('mouseout', this, false);
			}*/
		},

		refreshSize: function () {
			this.contenedorImagenGaleriaWidth = this.contenedorImagenGaleria.clientWidth;
			this.contenedorImagenGaleriaHeight = this.contenedorImagenGaleria.clientHeight;
			this.pageWidth = this.contenedorImagenGaleriaWidth;
			this.maxX = -this.options.numberOfPages * this.pageWidth + this.contenedorImagenGaleriaWidth;
...
/*			if (newX > 0 || newX < this.maxX) {
		...
		__event: function (type) {
			...
			this.contenedorImagenGaleria.dispatchEvent(ev);
		}
	};
